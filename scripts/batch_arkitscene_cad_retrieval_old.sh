...
# --- 统计功能设置 ---
STATUS_DIR="/tmp/batch_arkitscene_status"
mkdir -p "$STATUS_DIR"
...
#        return 0
#    fi
    # 检查 'cad_object_label.txt' 文件
#    if [ -f "$workspace/intermediate/HOC_Search/CAD_retrieval/cad_object_label.txt" ]; then
#        echo "[GPU ${gpu_id}] 检测到 'cad_object_label.txt'，跳过: $workspace"
#        touch "$STATUS_DIR/$(basename "$workspace").success"
#        return 0
#    fi
    ...
    python HOC_search/CAD_retrieval_HOC_search_ArkitScene.py \
        --config ArkitScene_HOC_Search.ini \
